...
- (void)viewDidLoad
{
    ...
    if ([self.callingView isEqualToString:@"InfoSchedules"]) {
        ...
        htmlString = @"Welcome to the newest version of the \"Now What\" application!  This version contains several new features.  Multiple schedules can now be viewed on the same device.  Create a new schedule by clicking the + button at the top right.  Events created in each schedule are unique to the schedule.  Templates are shared between all schedules.";
        
        [infoWebView loadHTMLString:htmlString baseURL:[NSURL URLWithString:@""]];
        
    }
    
    if ([self.callingView isEqualToString:@"InfoSaveTemplates"]) {
        
        htmlString = @"From this view, you can save a list of events as a template.  To add new events to this template, click the add button (+).  New events will show up in the template, but not be changed in your daily event list.  Events can be edited by clicking on them.";
